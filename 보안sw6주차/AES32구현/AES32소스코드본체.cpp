#include "AES32.h"
#include <iostream>
//static 정적 변수, 전역 변수, 고정영역에 저장, 파일내에서만 접근 가능한 변수(이름, 충돌을 방지)
// 지역변수 - 스택(함수내 변수) or 힙(메모리 할당)에 저장
static u32 Te0[256] =
...
void AES32_round(u32 state[4], u32 rk[4]) {
	...
	//tmp[0] = Te0[a00]^Te1[a11]^Te2[a22]^Te3[a33]^rk[0]
	tmp[0] = Te0[state[0] >> 24] ^ Te1[(state[1] >> 16)&0xff] ^ Te2[(state[2] >> 8)&0xff] ^ Te3[state[3]&0xff] ^ rk[0];
	
	//tmp[1] = Te0[a01]^Te1[a12]^Te2[a23]^Te3[a30]^rk[1]
	...
	tmp[2] = Te0[state[3] >> 24] ^ Te1[(state[0] >> 16) & 0xff] ^ Te2[(state[1] >> 8) & 0xff] ^ Te3[state[2] & 0xff] ^ rk[3];
