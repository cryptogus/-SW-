...
//AES 32bit Tables: Te0, Te1, Te2, Te3, Te4
//
//Te0[x] = [2*S(x), S(x), S(x), 3*S(x)]
//Te1[x] = [3*S(x), 2*S(x), S(x), S(x)]
//Te2[x] = [S(x), 3*S(x), 2*S(x), S(x)]
...
void AES32_Enc_Table_generation() {
	...
	for (int i = 0; i < 256; i++) {
		...
		Te0[i] = value;
		Te1[i] = (Te0[i] >> 8) ^ ((Te0[i] & 0xff) << 24);
		Te2[i] = (Te1[i] >> 8) ^ ((Te1[i] & 0xff) << 24);
		Te3[i] = (Te2[i] >> 8) ^ ((Te2[i] & 0xff) << 24);
