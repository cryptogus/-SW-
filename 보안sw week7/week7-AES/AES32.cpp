...
void AES32_round(u32 state[4], u32 rk[4]) {
	...
	//for (int k = 0; k < 4; k++) {
	//	tmp[k] = Te0[state[k] >> 24] ^ Te1[(state[(k+1)%4] >> 16) & 0xff] ^ Te2[(state[(k+2)%4] >> 8) & 0xff] ^ Te3[state[(k+3)%4] & 0xff] ^ rk[k];
	//}
	tmp[0] = Te0[state[0] >> 24] ^ Te1[(state[1] >> 16) & 0xff] ^ Te2[(state[2] >> 8) & 0xff] ^ Te3[state[3] & 0xff] ^ rk[0];
	tmp[1] = Te0[state[1] >> 24] ^ Te1[(state[2] >> 16) & 0xff] ^ Te2[(state[3] >> 8) & 0xff] ^ Te3[state[0] & 0xff] ^ rk[1];
	tmp[2] = Te0[state[2] >> 24] ^ Te1[(state[3] >> 16) & 0xff] ^ Te2[(state[0] >> 8) & 0xff] ^ Te3[state[1] & 0xff] ^ rk[2];
	tmp[3] = Te0[state[3] >> 24] ^ Te1[(state[0] >> 16) & 0xff] ^ Te2[(state[1] >> 8) & 0xff] ^ Te3[state[2] & 0xff] ^ rk[3];
...
void AES32_EqInv_round(u32 state[4], u32 rk[4]) {
	u32 tmp[4];
	// InvShiftRows() 적용한 원리를 이해하기 바랍니다.
	tmp[0] = Td0[state[0] >> 24] ^ Td1[(state[3] >> 16) & 0xff] ^ Td2[(state[2] >> 8) & 0xff] ^ Td3[state[1] & 0xff] ^ rk[0];
	tmp[1] = Td0[state[1] >> 24] ^ Td1[(state[0] >> 16) & 0xff] ^ Td2[(state[3] >> 8) & 0xff] ^ Td3[state[2] & 0xff] ^ rk[1];
	tmp[2] = Td0[state[2] >> 24] ^ Td1[(state[1] >> 16) & 0xff] ^ Td2[(state[0] >> 8) & 0xff] ^ Td3[state[3] & 0xff] ^ rk[2];
	tmp[3] = Td0[state[3] >> 24] ^ Td1[(state[2] >> 16) & 0xff] ^ Td2[(state[1] >> 8) & 0xff] ^ Td3[state[0] & 0xff] ^ rk[3];
...
void AES32_Encrypt(byte pt[16], u32 rk[11][4], byte ct[16]) {
	...
	//for (int k = 0; k < 4; k++) {
	//	tmp[k] = (Te4[state[k] >> 24] & 0xff000000) ^ (Te4[(state[(k+1)%4] >> 16) & 0xff] & 0x00ff0000)
	//		^ (Te4[(state[(k+2)%4] >> 8) & 0xff] & 0x0000ff00) ^ (Te4[state[(k+3)%4] & 0xff] & 0x000000ff) ^ rk[10][k];
	//}
	//마지막 라운드(10R)
...
void AES32_EqDecrypt(byte ct[16], u32 rk[11][4], byte pt[16]) {
	...
	tmp[1] = (Td4[state[1] >> 24] & 0xff000000) ^ (Td4[(state[0] >> 16) & 0xff] & 0x00ff0000)
		^ (Td4[(state[3] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[state[2] & 0xff] & 0x000000ff) ^ rk[0][1];
	tmp[2] = (Td4[state[2] >> 24] & 0xff000000) ^ (Td4[(state[1] >> 16) & 0xff] & 0x00ff0000)
		^ (Td4[(state[0] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[state[3] & 0xff] & 0x000000ff) ^ rk[0][2];
	tmp[3] = (Td4[state[3] >> 24] & 0xff000000) ^ (Td4[(state[2] >> 16) & 0xff] & 0x00ff0000)
		^ (Td4[(state[1] >> 8) & 0xff] & 0x0000ff00) ^ (Td4[state[0] & 0xff] & 0x000000ff) ^ rk[0][3];
	state2byte(tmp, pt);
}
...
void AES32_Enc_KeySchedule(byte k[16], u32 rk[11][4]) {
	...
	for (int i = 1; i < 11; i++) {
		...
		for (int k = 0; k < 4; k++) {
			if (k == 0) {
				tmp = SubWord(RotWord(tmp)) ^ Rcon[rcon_counter]; // tmp = F(tmp)
				rcon_counter++;
			}
			rk[i][k] = rk[i - 1][k] ^ tmp;
			tmp = rk[i][k];
